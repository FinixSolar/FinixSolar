...
const BulkUpload = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            setSuccess(`Successfully uploaded ${ data.length } records to '${ tableName }'.`);
            setFile(null);
            setData([]);
            setTableName("");
        } catch (err) {
            ...
            <main className="max-w-4xl mx-auto pt-32 pb-16 px-6">
                <div className="flex flex-col items-center mb-12">
                    ...
                    <p className="text-gray-400 mt-2 text-center max-w-md">
                        Efficiently import your Excel data into Supabase tables with a few clicks.
                    </p>
                </div>
                        ...
                        <div className="p-8 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl relative overflow-hidden group">
                            ...
                            <label className="cursor-pointer">
                                <input
                                    ...
                                    className="hidden"
                                />
                                <div className={`border-2 border-dashed rounded-2xl p-8 flex flex-col items-center justify-center transition-all ${ file ? 'border-blue-500 bg-blue-500/5' : 'border-white/10 hover:border-white/30 hover:bg-white/5' }`}>
                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${ file ? 'bg-blue-500 text-white' : 'bg-white/5 text-gray-400' }`}>
                    ...
                    <div className="flex flex-col h-full">
                        <div className="flex-1 p-8 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl flex flex-col justify-between">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="w-full bg-white/5 rounded-full h-1.5 overflow-hidden">
                                            <div
                                                ...
                                                style={{ width: `${ data.length > 0 ? (stats.processed / stats.total) * 100 : 0 }%` }}
                                            ></div>
                                        </div>
                                        {loading && (
                            ...
                            <button
                                ...
                                className={`w-full py-4 rounded-2xl font-bold text-sm tracking-widest uppercase transition-all flex items-center justify-center space-x-2 mt-8 ${ loading || !file || !tableName
                ...
                <div className="mt-12 p-6 rounded-3xl bg-blue-600/5 border border-blue-500/10">
                    ...
                    <ul className="text-sm text-gray-500 space-y-2 list-disc pl-5">
                        <li>Ensure the Excel column headers match the Supabase table column names exactly.</li>
                        <li>Format dates as <code className="text-blue-300">YYYY-MM-DD</code> for consistency.</li>
                        <li>Avoid empty rows at the end of your sheet for cleaner imports.</li>
                    </ul>
                </div>
            </main>
