        ...
        <div className="glass-card rounded-2xl shadow-xl p-8 transition-all duration-300">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-xs mt-1.5 text-center">
              Register to access the client management portal
            </p>
          </div>

          {success ? (
            <div className="text-center py-4 space-y-6">
              <div className="mx-auto w-16 h-16 rounded-full bg-blue/5 border border-blue/20 flex items-center justify-center text-blue dark:text-sky text-2xl">
                <i className="fa-solid fa-circle-check"></i>
              </div>
              <div>
                <h3 className="text-base font-bold text-navy dark:text-white">Verify your email</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">
                  We sent a confirmation link to <strong className="text-navy dark:text-slate-200">{email}</strong>. 
                  Please check your inbox to activate your account.
                </p>
              </div>
              <Link
                href="/login"
                className="inline-block w-full py-3 px-4 bg-gradient-to-r from-blue to-sky hover:opacity-95 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-lg transition-all duration-200 text-center"
              >
                Go to Login
              </Link>
            </div>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label htmlFor="fullName" className="block text-xs font-bold text-navy dark:text-slate-300 mb-2">
                    Full Name
                  </label>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 text-center">
                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                  ...
                  <Link href="/login" className="text-blue dark:text-sky hover:underline font-bold transition duration-150">
                    Log In
                  </Link>
                </p>
              </div>
