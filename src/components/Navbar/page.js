...
const Navbar = () => {
  ...
  const linkClass = (path) => {
    ...
    return `px-4 h-full flex items-center text-sm font-semibold transition-all duration-200 border-b-2 ${
      isActive
        ? "text-blue border-blue dark:text-sky dark:border-sky bg-blue/5 dark:bg-sky/5"
        : "text-slate-600 dark:text-slate-300 border-transparent hover:text-blue hover:border-blue/50 dark:hover:text-sky dark:hover:border-sky/50"
    }`;
  };
                  ...
                  {dropdownOpen && (
                    <div className="absolute right-0 mt-3.5 w-60 bg-white dark:bg-navy border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl py-2 z-50 animate-fade-in">
                      <div className="px-4 py-2.5 border-b border-slate-100 dark:border-slate-800">
                        ...
                        <p className="text-xs font-semibold text-navy dark:text-slate-200 truncate mt-0.5">{user.email}</p>
                      </div>
                      <Link
                        href="/dashboard"
